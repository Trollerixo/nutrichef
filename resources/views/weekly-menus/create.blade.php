<x-app-layout>
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            <h1 class="fw-bold h2 mb-1">Crear menú semanal</h1>
            <p class="text-muted mb-0">Define un nuevo menú con recetas y fechas para tu semana.</p>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('weekly-menus.store') }}" method="POST">
                @csrf
                ...
                @php
                    $slotRows = old('slots', [['slot_date' => '', 'meal_type' => '', 'recipe_id' => '']]);
                @endphp
                ...
                <button type="submit" class="btn btn-dark">Crear menú semanal</button>
            </form>
        </div>
    </div>
    ...
    (function () {
        ...
        function getDuplicateWarning(row) {
            ...
            const existing = slotsContainer.querySelectorAll('.slot-row');
            let count = 0;
            existing.forEach(r => {
                if (r === row) return;
        ...
        slotsContainer.addEventListener('change', event => {
            if (event.target.matches('[data-slot-field="meal_type"]') || event.target.matches('[data-slot-field="slot_date"]')) {
                ...
                if (warning && warning.count > 0) {
                    ...
                    row.querySelector('.col-md-4:last-child')?.after ??
        ...
        addSlotButton.addEventListener('click', function (e) {
            ...
            // Check for duplicate meal_type on same date in existing rows
            let dupCount = 0;
            if (date && type) {
